...
class NewsRecommender(nn.Module):
    def __init__(self, news_encoder, user_encoder, click_predictor, loss_modules, ctr_predictor):
        ...
        self.beta = nn.Parameter(torch.tensor(0.5))  # learnable parameter
    ...
    def update_ctr(self, answer, candidates):
        ...
        for impression in candidates:
            ...
            if impression == 6865 or impression == 114054:
                print(self.news_impressions[impression])
                print("click", self.news_clicks[impression])
    ...
    def forward(self, candidate_news, clicked_news, labels, answer, candidates, negative_samples, history_ids, time, mask=None):
        ...
        for answer_id in answer:
            ...
            if len(answer_ctr) > 1:  # 최소 2개 이상 있어야 학습 가능
                answer_ctr_tensor = torch.tensor(answer_ctr[:-1]).unsqueeze(-1).to(self.device)  # [seq_len, 1]
                ...
                if next_ctr > 0.2:
                    ...
                    print("answer_ctr",answer_ctr)
                    print("predicted", predicted_next_ctr, "next_ctr", next_ctr)
        ...
        click_probability = (1 - self.beta) * click_probability + self.beta * torch.cat([ctr_answer_predicted, ctr_negative_samples_predictions], dim=1)
        recommendation_loss  = sum(
            [
                loss_module(
                    {
        ...
        total_loss = recommendation_loss + lstm_loss
        return total_loss
