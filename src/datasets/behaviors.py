...
def behaviors_collate_fn(batch):
    ...
    answer = [x[5] for x in batch]
    history_ids = [x[6] for x in batch]
    time = [x[7] for x in batch]
    return log_ids, clicked_news_vectors, mask, impression_ids, clicked, answer, history_ids, time
...
class BehaviorsDataset(Dataset):
    ...
    def __init__(
        ...
        self.behaviors = load_behaviors(
            mind_variant,
            splits=[self.split],
            columns=["log_id","time", "history", "impressions"],
        )
    ...
    def __getitem__(
        ...
        answer = 0
        if idx != 0:
            behaviors_row = self.behaviors.iloc[idx - 1]
            answer = [int(item[1:-2]) for item in behaviors_row["impressions"] if item.endswith("-1")][0]
        ###########################################
        history_ids = [int(item[1:]) for item in row["history"]]
        if len(history_ids) < self.history_length:
            # Add padding (0) to the front
            history_ids = [0] * (self.history_length - len(history_ids)) + history_ids
        else:
            # Truncate from the back
            history_ids = history_ids[-self.history_length:]
        time = row.time.timestamp()
        return row.log_id, clicked_news_vectors, mask, impression_ids, clicked, answer, history_ids, time
